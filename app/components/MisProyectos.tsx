...
const ProjectCard: React.FC<{ title: string; description: string; repoLink: string }> = ({ title, description }) => {
  return (
    <main className="mb-12">
      <h3 className="text-xl font-serif mb-3 text-black dark:text-white font-semibold mt-10">{title}</h3>
      <p className="text-sm mb-4 leading-relaxed text-gray-800 dark:text-gray-300 font-serif">{description}</p>
      <a
  ...
  const projects = [
    {
      ...
      description:
        "Desarrollé una interfaz funcional y sencilla conectada a una base de datos con Django para gestionar dispositivos vinculados al hogar. El sistema permite controlar luces revisar las cámaras y un sistema de bloqueo todo, con una interfaz amigable. Ideal para automatizar tareas cotidianas, integrando tecnología con comodidad.",
      repoLink: "#",
    },
    {
      ...
      <header className="max-w-6xl mx-auto px-6">
        <main className="grid grid-cols-1 md:grid-cols-2 gap-x-16">
          ...
          <main className="mt-8 md:mt-1">
            <p className="text-lg font-serif mb-8 text-black dark:text-gray-300 font-semibold">
              Una selección de proyectos personales que he realizado
            </p>
